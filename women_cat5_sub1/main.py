...
# Hardcoded subcategory URLs for Fragrances Group 1
SUBCATEGORY_URLS = [
    "https://www.boutiqaat.com/ar-kw/women/fragrances/perfumes-1/l/",
    "https://www.boutiqaat.com/ar-kw/women/fragrances/hair-mist/l/",
    "https://www.boutiqaat.com/ar-kw/women/fragrances/body-mists/l/",
    "https://www.boutiqaat.com/ar-kw/women/fragrances/candles-home-scents/l/",
]
...
class BoutiqaatDataPipeline:
    ...
    def _process_category(self, scraper: BoutiqaatScraper, category: dict) -> bool:
        ...
        try:
            ...
            if not products:
                logger.warning(f"No products found for {category_name} — skipping (empty category)")
                return True
    ...
    def _upload_product_image(self, product: Dict, category_name: str, subcategory_name: str) -> str:
        ...
        try:
            ...
            s3_path = (
                f"boutiqaat-data/year={datetime.now().strftime('%Y')}/month={datetime.now().strftime('%m')}/day={datetime.now().strftime('%d')}/fragrances/images/"
                f"{safe_category}"
            )
    ...
    def _upload_excel_file(self, local_path: str, category_name: str) -> bool:
        ...
        try:
            ...
            s3_path = (
                f"boutiqaat-data/year={datetime.now().strftime('%Y')}/month={datetime.now().strftime('%m')}/day={datetime.now().strftime('%d')}/fragrances/excel-files"
            )
